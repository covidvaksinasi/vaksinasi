...
if ($_GET['id']) {
  ...
  $sqlpkt = mysql_query("SELECT * FROM tindakan order by kode_tindakan");
  ...
  while ($rhasil = mysql_fetch_array($sqlhasil)) {
    $artindakan = unserialize($rhasil['tindakan']);
...
// // --------------------- END -------------------------
  ...
  echo "<div class='callout callout-default'>Jenis tindakan yang diderita adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1], 2)*100 . "%<br></h3>";
